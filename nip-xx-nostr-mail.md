...
kind 1400 rumors that are sealed and gift-wrapped before publication. Metadata
...
- **No attachment standard.** File sharing via kind 1400 (NIP-17) provides raw
...
appear in capitalized form, as shown here.
...
| 30099 | Mailbox State | Read/flag/folder state | Addressable (by `d` tag) |
...
A kind 1400 event is an unsigned rumor (per NIP-59) containing the mail message.
...
Every kind 1400 rumor MUST include exactly one `subject` tag. The subject text
...
| `text/html` | HTML content. Clients SHOULD NOT send this type due to the substantial security burden. Clients that receive `text/html` MUST sanitize before rendering. See [Security Considerations](#html-sanitization). New implementations MAY omit HTML rendering support. |

##### Message ID Tag

```
["message-id", <random-hex>]
```

Every kind 1400 rumor MUST include exactly one `message-id` tag containing a
32-byte random hex string generated by a CSPRNG. This value serves as the
stable identity of the message across all recipients and is used for threading,
mailbox state tracking, and deduplication.

The `message-id` is necessary because the gift-wrap event ID differs for each
recipient (each recipient receives a unique gift wrap with a unique ephemeral
key). Without a shared identifier inside the encrypted rumor, multi-recipient
threads would fracture — a reply referencing one recipient's gift-wrap ID
would be unresolvable by other recipients.

When creating both a visible rumor (for To/CC) and a BCC rumor (see
[Recipients](#recipients-to-cc-bcc)), the sender MUST use the **same**
`message-id` value in both rumor variants. This ensures BCC recipients can
participate in threads without fracturing the conversation.

##### Threading Tags

```
["reply", <message-id>, <relay-hint>]
["thread", <message-id>, <relay-hint>]
```
...
reference the `message-id` value of the target message (NOT the gift-wrap
event ID, which differs per recipient).
...
```json
{
  ...
  "tags": [
    ...
    ["subject", "Hello"],
    ["message-id", "b1a2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2"]
  ],
...
```json
{
  ...
  "tags": [
    ...
    ["subject", "Re: Hello"],
    ["message-id", "c2d3e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3"],
    ["reply", "b1a2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2", "wss://relay.alice.com"],
    ["thread", "b1a2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2", "wss://relay.alice.com"]
  ],
...
```json
{
  ...
  "tags": [
    ...
    ["message-id", "d3e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4"],
...
```json
{
  ...
  "tags": [
    ...
    ["message-id", "e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5"],
...
1. **Rumor (kind 1400)** -- The unsigned mail message. Contains the sender's
...
When serializing a kind 1400 rumor to JSON for NIP-44 encryption,
...
Recipient roles are encoded in the `p` tags of the kind 1400 rumor.
...
1. Create a lookup table mapping each message's `message-id` tag value to the
   message.
2. For each message with a `reply` tag:
   a. If the referenced parent `message-id` exists in the lookup table, link
      the message as a child of that parent.
...
Messages are grouped into conversations by their `thread` tag value (`message-id`
of the root). Messages without a `thread` tag that are not referenced as a
parent by any other message form single-message threads keyed by their own
`message-id`.
...
Implementations SHOULD limit the number of partitions fetched to a reasonable
window (e.g., the last 12 months) and MAY compact older partitions by removing
entries for messages the user has permanently deleted.
...
Kind 30099 is an addressable event. Within each partition (same `d` tag),
when a client receives multiple versions, it MUST keep the one with the latest
`created_at`. If two state events for the same partition have the same
`created_at`, the event with the lexicographically lower `id` (lowercase hex
comparison) MUST be kept. This tiebreaker rule extends NIP-01's addressable
event semantics, which do not define behavior for equal timestamps.
...
After merging, the client SHOULD publish a new kind 30099 event for each
modified partition containing the merged state.
...
A kind 30016 addressable event stores an encrypted draft message. The `d` tag
...
Implementations MUST NOT include a `subject` tag on the outer (unencrypted)
draft event. The subject is part of the encrypted content and exposing it on
the outer event leaks metadata to relays. Implementations that need to list
drafts without decryption SHOULD maintain a local index.
...
Step 1: Create the kind 1400 rumor (unsigned)
  - Set pubkey to Alice's real pubkey
  - Generate a random 32-byte hex message-id via CSPRNG
  - Add ["message-id", <random-hex>]
...
Step 5: Process the message
  - Extract the message-id tag for stable identity
  - Parse tags: subject, recipients, threading, attachments
  - Store in the appropriate folder based on tier evaluation
  - Update kind 30099 state if needed (in the partition for current month)
...
- Relays SHOULD NOT inspect, decrypt, or log the content of kind 1059 events.
  The protocol's security model assumes relays are honest-but-curious;
  NIP-44 encryption ensures message confidentiality even if this expectation
  is violated.
- Relays MUST store kind 10097, 30099, and 30016 events per their respective
...
- Clients MUST use NIP-59 gift wrapping for all kind 1400 and kind 1401 events.
...
- Clients MUST include at least one `p` tag, one `subject` tag, and one
  `message-id` tag in every kind 1400 rumor.
- Clients MUST generate `message-id` values using a CSPRNG (32-byte hex).
- Clients MUST use the same `message-id` value across visible and BCC rumor
  variants of the same message.
- Clients MUST look up recipient inbox relays via kind 10050, falling back to
  kind 10002 (NIP-65).
- If a kind 1400 rumor is received without a `subject` tag, clients MUST
  treat the subject as empty and MUST NOT reject the message solely for this
  reason.
...
- Clients SHOULD deduplicate received messages by `message-id` tag value.
...
- Clients SHOULD introduce a random publication delay (0-60 seconds) before
  publishing each gift wrap to mitigate timing correlation.
...
NIP-05 reverse lookup or a local mapping table), constructs a kind 1400 rumor
...
**Layer 1 -- Rumor (kind 1400, unsigned):**
```json
{
  "kind": 1400,
  "pubkey": "2c7cc62a697ea3a7826521f3fd34f0cb273693cbe5e9310f35449f43622a6748",
...
**Rumor serialized as JSON string (input to NIP-44 encryption):**
```
{"kind":1400,"pubkey":"2c7cc62a697ea3a7826521f3fd34f0cb273693cbe5e9310f35449f43622a6748","created_at":1711843200,"tags":[["p","98b30d5bfd1e2e751d7a57e7a58e67e15b3f2e0a90f9f7e8e40f7f6e5d4c3b2a","","to"],["subject","Hello"],["message-id","b1a2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2"]],"content":"Hi Bob, how are you?","sig":""}
```

**NIP-44 conversation key (seal layer):**
```
ECDH(alice_privkey, bob_pubkey)
  alice_privkey = 7f7ff03d123792d6ac594bfa67bf6d0c0ab55b6b1fdb6249303fe861f1ccba9a
  bob_pubkey    = 98b30d5bfd1e2e751d7a57e7a58e67e15b3f2e0a90f9f7e8e40f7f6e5d4c3b2a
```

Note: The actual encrypted ciphertext is non-deterministic due to NIP-44's
random nonce. Implementations MUST verify round-trip correctness: encrypt the
rumor, decrypt it, and confirm all fields match. The companion test vectors in
`test-vectors/gift-wrap.json` provide concrete ciphertext values generated by
the reference implementation for validation.

**Layer 2 -- Seal (kind 13, signed by Alice):**
```json
{
  "id": "<sha256-of-serialized-seal>",
  "pubkey": "2c7cc62a697ea3a7826521f3fd34f0cb273693cbe5e9310f35449f43622a6748",
  "created_at": 1711756800,
  "kind": 13,
  "tags": [],
  "content": "<NIP-44 encrypted rumor JSON using ECDH(alice, bob)>",
  "sig": "<schnorr-signature-by-alice>"
}
```
Note: `created_at` is randomized (differs from rumor by up to ±2 days). Tags
are always empty. The seal is signed by Alice's real key — this is the ONLY
layer that cryptographically binds the sender.
...
The AES-256-GCM authentication tag MUST be 128 bits (16 bytes).
Implementations MUST NOT use a truncated authentication tag. Implementations
MUST verify the authentication tag during decryption and MUST reject data
where verification fails.

### Ephemeral Key Lifecycle

Implementations MUST zero or overwrite ephemeral private key material in
memory after signing the gift wrap event. While runtime garbage collection
may not guarantee immediate erasure, explicit zeroing (e.g.,
`ephemeralPrivkey.fill(0)` in JavaScript, `copy(key[:], zeroSlice)` in Go)
reduces the exposure window in memory dumps and heap inspections. Derived
conversation keys SHOULD also be zeroed after use.

### Spam Policy Metadata Exposure

Kind 10097 spam policy events are published unencrypted so that senders can
comply with recipient preferences before sending. This inherently exposes the
recipient's anti-spam configuration: the Cashu threshold, accepted mints, and
contact-bypass setting are visible to anyone. An attacker can use this
information to craft messages that precisely meet the minimum requirements.

Recipients SHOULD be aware that their spam policy is public metadata.
Implementations SHOULD make the minimum Cashu amount setting prominent in the
UI and SHOULD advise recipients that the default of 10 sats may be
insufficient for high-profile users. At current Bitcoin prices (~$65K/BTC),
10 sats equals approximately $0.0065 per message, meaning 100,000 spam
messages cost only ~$650. Recipients who receive significant unsolicited
contact SHOULD consider setting `cashu-min-sats` to 100 or higher.

### Publication Timing

Timestamp randomization (applied to the `created_at` field on seal and gift
wrap layers) does NOT protect against network-level timing correlation. A
relay operator observing when a gift wrap event arrives on the WebSocket can
correlate the arrival time with a sender's connection, regardless of the
randomized `created_at` value.

Implementations SHOULD introduce a random publication delay of 0 to 60
seconds (uniform random, CSPRNG) between constructing a gift wrap and
publishing it to relays. This adds a second layer of timing obfuscation.
Implementations MUST NOT delay by more than 300 seconds to avoid degrading
message delivery. Clients MAY additionally route gift wrap publication through
Tor or a mix network to further obscure the sender's network identity.
...
- This NIP does NOT affect existing NIP-17 direct messages. Kind 14 (DM) and
  kind 1400 (mail) are distinct event kinds with different semantics.
- Kind 1400 and kind 1401 events are always gift-wrapped as kind 1059 events.
...
- Clients that do not implement this NIP will see kind 1059 events but, upon
  decrypting the inner rumor, will encounter kind 1400 and can safely ignore it
  as an unrecognized kind.
- Kind 10097 (spam policy), 30099 (mailbox state), and 30016 (drafts) are new
...
### Category 1: Event Structure (12 tests)
...
| E04a | Missing subject tag handled gracefully | Absent subject treated as empty, message not rejected |
...
| E06a | Message-id tag is present and valid | `["message-id", <32-byte-hex>]` |
...
1. **Signing the rumor.** Kind 1400 and kind 1401 events MUST NOT be signed. The
